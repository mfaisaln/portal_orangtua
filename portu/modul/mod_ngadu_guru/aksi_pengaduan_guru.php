...
if (empty($_SESSION['id_pengguna']) AND empty($_SESSION['passuser'])) {
    ...
    //MODUL TAMBAH GURU
    if ($module == 'ngadu' AND $act == 'tambah') {
        //FUNGSI MENGHILANGKAN BANYAK SPASI DIAMBIL DARI fungsi/fungsi_validasi.php =>function trimed
        $ngadu = trimed($_POST['ngadu']);
        $id = $_SESSION['id_or'];

        $cek = mysql_query("SELECT ngadu FROM ngadu WHERE ngadu = '$ngadu'");
        if (mysql_num_rows($cek) > 0) {
            echo"<script language='javascript'>
                ...
                $query = mysql_query("INSERT INTO ngadu(
                                    ngadu,id_ortu)
                            VALUES(
                                    '$_POST[ngadu]','$id;')") or die(mysql_error());
                if ($query) {
    ...
    } elseif ($module == 'ngadu' AND $act == 'hapus') {
            //HAPUS DATA MERK
            $query = mysql_query("DELETE FROM ngadu WHERE id_ngadu='$_GET[id]'");
            if ($query) {
